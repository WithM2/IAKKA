...
public class MjpegStreamReader : MonoBehaviour
{
    ...
    [SerializeField]
    private string mjpegUrl = "http://192.168.1.1/mjpeg/1"; // MJPEG 스트림 URL 설정
    public RawImage rawImageDisplay; // UI 요소로 MJPEG 스트림을 표시할 RawImage
    public Texture2D texture; // 스트림 이미지를 표시할 텍스처
    ...
    void OnEnable()
    {
        ...
        {
            streamThread = new Thread(StreamMJPEG);
            streamThread.Start();
        }
    }
    ...
    void OnDisable()
    {
        ...
        if (streamThread != null && streamThread.IsAlive)
        {
            streamThread.Join(500); // 스레드가 종료될 때까지 대기
            streamThread = null; // 스레드 참조 해제
        }
    }

    void StreamMJPEG()
    {
        Debug.Log("StreamMJPEG");
        ...
        request.Timeout = 5000; // 시간 초과를 무한으로 설정하여 스트림 지속 가능
